...
- 2026-04-18: critic loop §12.2 i18n - `frontend/components/ui/search-input.tsx` atom hardcoded `placeholder = "Search"` as its destructure default. Any caller relying on the default (design-system demo page is the current one; new callers are free to skip the prop) silently shipped the English word "Search" to French users regardless of locale routing. Fixed by moving the default off the destructure and computing `resolvedPlaceholder = placeholder ?? t("search")` inside the component body, following the same pattern already in use by `multi-select-pill.tsx` and `resources-filter-bar.tsx`. The `search` key already exists in both `messages/en.json` ("Search") and `messages/fr.json` ("Rechercher"); i18n-parity vitest still green (3 tests, leaves in lock-step). Callers still free to override via explicit `placeholder` prop.
- 2026-04-18: critic loop §12.2+§12.3 a11y i18n - `frontend/components/ui/spinner.tsx` atom hardcoded `label = "Loading"` as a destructure default, and the default value is piped directly to `aria-label`. Every screen reader listening to a Spinner that did not pass an explicit `label` heard the English word "Loading" regardless of the user's locale (a French user on fr.gammahr.com heard "Loading" instead of "Chargement"). The existing `no_hardcoded_aria_labels.py` hook does not catch this class because `aria-label={label}` is a JSX expression on a variable; the default value is invisible to the string-literal regex. Fixed the same way as `search-input.tsx`: move the default off the destructure, compute `resolvedLabel = label ?? t("loading")` with `useTranslations("app")` (the `app.loading` key already exists in both `en.json` "Loading" and `fr.json` "Chargement", so no i18n tree mutation is needed and parity stays green at 1191 leaves). Callers still free to override via explicit `label` prop. `tsc --noEmit` clean on the atom.
