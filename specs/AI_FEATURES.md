...
> **Providers, per tier (see `docs/decisions/ADR-011-ai-vendor-ollama.md`):** Production runs **Vertex AI Gemini 2.5 Flash** (EU region `europe-west9`) via the Google Cloud AI Platform Python SDK. Dev runs **self-hosted Ollama** (Gemma3) on localhost so customer data never leaves the developer's machine. CI and unit tests use **MockAIClient**. The vendor is selected at boot via `settings.ai_backend`; `tier=prod` refuses to boot with any backend other than Vertex. All code lives in `backend/app/ai/` behind the single `AIClient` abstraction (M1), plus per-feature tool definitions in `backend/app/features/*/ai_tools.py`.
