...
- **Cross-tenant schema drift is a real operational problem.** A migration can fail on one tenant without failing on others, leaving that tenant behind on schema version. Mitigated by a weekly Celery job that fingerprints each tenant's schema and alerts on drift. Full auto-reconciliation UI deferred (DEF-054).
...
- **Recovery runbook** (`docs/ROLLBACK_RUNBOOK.md`) covering per-tenant rollback and full-cluster PITR.
