...
- **Cold start mitigation (detail).** Production Cloud Run services (ops, app, portal, worker) are configured with `min_instances = 1` at all times. Staging can use `min_instances = 0` (cold starts acceptable in non-customer environments). Expected cold start is 2 to 4 seconds with Python 3.12 + FastAPI; warm p95 < 500 ms. Load test target: first login after weekend warm-up, p95 < 5 seconds cold, p95 < 500 ms warm. If cold-start exceeds 5 seconds, add a Cloud Scheduler pre-warm job (hit `/health` every 4 minutes) before increasing min_instances.
- **Cloud SQL HA.** Regional HA (primary + standby in different zones in europe-west9). Failover SLO: RTO < 120 seconds, RPO < 1 transaction (synchronous replication). On failover, the Cloud SQL Auth Proxy reconnects automatically; SQLAlchemy sessions get `OperationalError` once, then reconnect. Client retry policy in `lib/api-client.ts`: on 503, exponential backoff 1s/2s/4s/8s/16s (5 attempts, total ~31s), then surface a banner "Reconnecting to the database..." and continue retrying. Failover drill is run monthly in staging via `gcloud sql instances failover`.
- **Deployment pipeline.** Staging deploys auto on merge to `main`. Production deploys are manual: founder clicks Approve in the GitHub Actions UI after staging smoke tests pass. Blue/green via Cloud Run revision traffic split: new revision deploys at 0% traffic, smoke test hits `/health` and schema version, if OK traffic moves to 100% in a single command (`gcloud run services update-traffic --to-latest`). Rollback: `gcloud run services update-traffic --to-revisions=PREVIOUS=100` reverts in ~5 seconds. The previous revision is kept warm (min instances 1) for 24 hours before scale-down.
